  ...
  function(palette, mustache) {
    ...
    stamppalette.StampPalette = function(invoker, primaryText) {
      ...
      this.template =
        ...
        '{{#.}}' +
        '<td>' +
        '<button base="{{stampBase}}" proportionnal="{{proportionnal}}" value="{{stamp}}" style="height:55px; width:55px; background-size:40px; background-image: url({{ stamp }}); background-repeat: no-repeat; background-position: center; "></button>' +
        '</td>' +
        '{{/.}}' +
        '</tr>' +
        '{{/rows}}' +
